...
export default function TechCalibration() {
  ...
  // FIX: also destructure calibrationLog so the done-screen can render it
  const { addCalibrationEntry, calibrationLog } = useClinic();
  ...
  const CALIB_FALLBACK = {
    overall: "Pass with Advisory",
    quality_score: 97.4,
    advisory: "Gradient Coil B shows minor deviation (0.02%) within accepted tolerance. Automatic shimming applied.",
    clearance: "Approved for Clinical Use",
    sign_off_note: "Unit cleared for diagnostic deployment. Monitor Coil B metrics on next scheduled service.",
  };
  ...
  const runAI = async () => {
    ...
    const aiPromise = askMedGemmaJSON(
      ...
      `Unit: Siemens Magnetom Vida 3T · RX-990. Scan: ${HW_SYSTEMS.map(h => h.label + ':' + h.val + '%').join(', ')}.`,
      CALIB_FALLBACK
    );
  ...
  const submitCert = () => {
    ...
    setTimeout(() => {
      addCalibrationEntry({
        unit:      'Siemens Magnetom Vida 3T · RX-990',
        tech:      sig,
  ...
  if (done) return (
    <div className="max-w-2xl mx-auto pt-24 text-center pb-20">
      ...
      <p className="text-sm font-bold opacity-50 uppercase text-primary mb-8">
        Siemens Magnetom Vida 3T · RX-990 is certified for clinical use. Record filed under your Tech ID.
      </p>

      {/* FIX: calibrationLog is now properly destructured from useClinic above */}
          ...
          {calibrationLog.slice(0, 3).map(entry => (
            <div key={entry.id} className="p-5 bg-emerald-50 border border-emerald-200 rounded-2xl">
              <div className="flex justify-between items-start">
                ...
                <span className="text-[9px] font-black bg-emerald-500 text-white px-3 py-1 rounded-full uppercase">
      ...
      <div className="flex gap-4 justify-center">
        <button onClick={() => navigate('/tech/dashboard')}
        ...
        <button onClick={() => navigate('/tech/service-orders')}
          className="glass-panel px-10 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest text-primary">
          Service Orders
        </button>
      </div>
    </div>
            ...
            <div className={glass}>
              ...
              <div className="h-44 bg-primary/5 rounded-[2.5rem] border border-white/50 relative overflow-hidden mb-8">
                ...
                <svg className="w-full h-full px-6 py-4" viewBox="0 0 100 100" preserveAspectRatio="none">
                  ...
                  <polyline
                    points={wavePoints.split(' ').map(pt => {
                      const [x, y] = pt.split(',');
                      return `${x},${+y * 0.6 + 20}`;
                    }).join(' ')}
